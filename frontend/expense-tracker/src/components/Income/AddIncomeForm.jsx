...
import { LuDollarSign, LuCalendar, LuBriefcase, LuImage, LuUtensils, LuUpload } from 'react-icons/lu'
import Input from '../Inputs/Input'
import axiosInstance from '../../utils/axiosInstance'
import { API_PATHS } from '../../utils/apiPaths'

const AddIncomeForm = ({ onSuccess, onCancel }) => {
  console.log('AddIncomeForm props:', { onSuccess: !!onSuccess, onCancel: !!onCancel });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitData = {
        ...
        date: formData.date,
        ...(formData.icon && { icon: formData.icon }) // Only include icon if it exists
      }

      const response = await axiosInstance.post(API_PATHS.INCOME.ADD_INCOME, submitData)
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="flex flex-col items-center">
          ...
          {!formData.icon ? (
            <div className="w-16 h-16 flex items-center justify-center bg-teal-100 rounded-full relative">
              <LuUtensils className="text-2xl text-teal-600" />
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                className="w-6 h-6 flex items-center justify-center bg-teal-500 text-white rounded-full absolute -bottom-1 -right-1 hover:bg-teal-600 transition-colors"
              >
      ...
      <div>
        ...
        <Input
          ...
          placeholder="e.g., Salary, Freelance, Investment, etc."
        />
      </div>
        ...
        <button
          ...
          onClick={() => {
            ...
            if (onCancel) {
              ...
              // Try to close modal by other means if onCancel is not provided
              window.history.back(); // This is a fallback, but not ideal
            }
          }}
          disabled={loading}
